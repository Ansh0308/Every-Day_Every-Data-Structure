...
void helper(string s,vvs& ans,vs temp={}){
    //base case
    if(s.size()==0){
        ans.push_back(temp);
        return;
    }
    //making cuts at all positions
    for(int i=0;i<s.size();i++){
        string part=s.substr(0,i+1);
        if(isPal(part)){
            temp.push_back(part);
            helper(s.substr(i+1),ans,temp);//calling for remaining string
            temp.pop_back();
        }
    }
}
vvs palindrome_partitions(string s){
    vvs ans;
    helper(s,ans);
    return ans;
...
void solve() {
    ...
    string s;
    cin >> s;
    vvs result = palindrome_partitions(s);
