...
void print_vec_vec(const vector<vector<int>>& v, string sep = " ") {
    for (auto &vec : v) {
        for (auto &x : vec) cout << x << sep;
        cout << endl;
    }
}
...
// 🔹 Main Logic Example
// -------------------------------
void helper(vi &nums,vector<vi> &ans, int index=0){
    if(index==sz(nums)){
        ans.pb(nums);
        return;
    }
    for(int i=index;i<sz(nums);i++){
        swap(nums[index],nums[i]);//swaping every element with index 
        helper(nums,ans,index+1);//swapping next index
        swap(nums[index],nums[i]);//backtracking
    }
}
vector<vi> permutations(vi &nums){
    vector<vi> ans;
    helper(nums,ans);
    return ans;
...
void solve() {
    ...
    int n; cin >> n;
    vi arr = input_vi(n);
    vector<vi> perms = permutations(arr);
