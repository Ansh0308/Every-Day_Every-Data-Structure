...
class LinkedList{
    ...
    Node* swapnodes_in_pairs(Node* head){
        if(!head || !head->next){
            return head;
        }
        Node* temp=head;
        Node* Next=temp->next;
        Node* after=swapnodes_in_pairs(temp->next->next);
        temp->next=after;
        Next->next=temp;
        return Next;
    }
    Node* reverseKGroup(Node* head, int k) {
        ...
        Node* after= reverseKGroup(curr, k); // Recursively reverse the remaining list
        ...
        return after; // New head of the reversed group
    }
...
void solve() {
    ...
    ll.head = ll.swapnodes_in_pairs(ll.head);
    ll.printList();
